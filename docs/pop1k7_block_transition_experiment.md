...
```text
Pop1K7由来の1000個の異なるMIDIファイルを processed 化し、合計約143万イベントの時系列データとして整備した。各データには、入力特徴 x、vMF方向 μ、root、コードテンプレート、拍節、velocity、timing、duration、和声機能 function、機能遷移 function_transition などを保存している。

このデータを用いて、4層・4ヘッドの Conformer-vMF block transition モデルを学習した。初期段階では Conformer 本体を凍結して block transition head を15 epoch 学習し、その後 fine-tuning を10 epoch 実施した。fine-tuning 後、検証精度は val_acc = 0.832、val_macro_f1 = 0.808 に到達した。また、head_best と finetune_best の差分比較により、input projection および 4層 Conformer blocks の重みも更新されていることを確認した。
```
